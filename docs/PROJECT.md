...
- [x] Phase 8: Performance and architecture hardening (hashing, payload split, progressive hydration)
...
## Milestone Checklist (Phase 8)
- [x] Derive build hash from generated output content (not only source file metadata).
- [x] Use real build timestamp for generated manifest/sitemaps.
- [x] Replace canonical legacy detail duplicates with lightweight redirect aliases.
- [x] Split supplemental data payloads (`about-data.json`, `tofu-data.json`) to avoid redundant heavy fetches.
- [x] Reuse in-memory home payload for tofu diagnostics when available.
- [x] Add progressive list hydration for heavy subgroup/search/tag card grids.
- [x] Expose tofu diagnostics route in shared menu/footer navigation.
- [x] Upgrade `test:lighthouse-budget` from byte-only proxy to include runtime vitals budgets.
- [x] Re-run full quality gates and sync docs.
...
| 2026-02-21 | Compute build hash from generated output content and current indexing config | Prevents stale service worker/manifest versions when output changes without grouped-data timestamp changes. |
| 2026-02-21 | Convert canonical legacy detail aliases into redirect stubs instead of full duplicate detail documents | Reduces output footprint and duplicate rendering overhead while preserving backward link compatibility. |
| 2026-02-21 | Split About/Tofu datasets and add progressive hydration for heavy collection routes | Reduces payload and DOM cost while preserving KISS card UX across browse pages. |
| 2026-02-21 | Add runtime vitals budgets to QA gate (`test:lighthouse-budget`) | Moves performance checks beyond static byte budgets to measured route behavior. |
