...
async function findRepoRoot(startDir: string): Promise<string> {
  ...
  for (;;) {
    try {
      await fs.access(path.join(dir, 'grouped-openmoji.json'));
      return dir;
    } catch {
      const parent = path.dirname(dir);
      if (parent === dir) {
        throw new Error(`Could not locate grouped-openmoji.json walking up from ${startDir}`);
      }
      dir = parent;
    }
  }
...
export async function loadEmojiModel(): Promise<EmojiModel> {
  ...
  // Find the repo root by walking up from cwd until grouped-openmoji.json
  // appears. Works whether the process runs from site/ (vite build, vitest)
  ...
  // The local SVG set (assets/emoji/base) does not cover every emoji, so
  // useLocalAsset must reflect actual file presence — anything else 404s.
  // Entries without a local file fall back to the OpenMoji CDN.
  const assetDir = path.join(repoRoot, 'assets', 'emoji', 'base');
  let localAssetFiles = new Set<string>();
  try {
    localAssetFiles = new Set(await fs.readdir(assetDir));
  } catch {
    // No local assets available: everything uses the CDN.
  }
  ...
  for (const [groupKey, subgroupObject] of groupEntries) {
    const subgroupEntries: [string, any[]][] = Object.entries(
      subgroupObject as Record<string, any[]>
    ...
    for (const [subgroupKey, emojis] of subgroupEntries) {
      for (const emoji of emojis) {
        ...
        const openmoji_tags: string[] =
          typeof openmojiTagsRaw === 'string'
            ? openmojiTagsRaw
                .split(',')
                .map((t: string) => t.trim())
                .filter(Boolean)
            : Array.isArray(openmojiTagsRaw)
              ? openmojiTagsRaw
              : [];
  ...
  for (const entry of emojiEntries) {
    ...
    entry.useLocalAsset = localAssetFiles.has(`${entry.assetHex}.svg`);
  }
