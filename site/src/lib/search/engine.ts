/**
 * Search engine ported from home-search.mjs.
 *
 * Faithful port of the existing 490-line search algorithm to TypeScript.
 * Tokenization, stemming, fuzzy matching, alias expansion, scoring weights
 * are all preserved exactly as-is.
 */

import { normalizeHex } from '../data/slug';
...
export interface EntrySearchIndex {
  ...
  hexCompact: string;
  emoji: string;
  annotationTokens: Set<string>;
  tagTokens: Set<string>;
  groupTokens: Set<string>;
  subgroupTokens: Set<string>;
...
function normalizeSearchValue(value = ''): string {
  return String(value).normalize('NFKD').replace(DIACRITIC_RE, '').toLowerCase().trim();
}
...
export function tokenizeSearch(value = '', { minLength = 1 } = {}): string[] {
...
function extractHexCandidates(rawQuery = ''): string[] {
...
function buildWeightedQueryTokens(_query: string, queryTokens: string[]): Map<string, number> {
  const weighted = new Map<string, number>();

  const addWeight = (token: string, weight: number): void => {
    if (!token) return;
...
function getEditDistanceThreshold(token: string): number {
  if (token.length >= 8) return 2;
  if (token.length >= 4) return 1;
  return 0;
}

function withinDistance(source: string, target: string, maxDistance = 1): boolean {
...
function hasWordBoundaryMatch(haystack: string, phrase: string): boolean {
...
function cleanTagTokens(tags: string): string[] {
  return tokenizeSearch(tags, { minLength: 1 }).filter(
    (token) => !STOPWORDS.has(token) && token.length > 1
  );
}

function buildPrefixedSet(tokens: string[]): Set<string> {
...
function findPrefixToken(token: string, tokenSet: Set<string>, minLength = 3): boolean {
...
function findFuzzyToken(token: string, tokenSet: Set<string>): boolean {
...
function stripVowels(token = ''): string {
  return String(token).replace(VOWEL_RE, '');
}

function findVowelDropToken(token: string, tokenSet: Set<string>): boolean {
...
export function buildEntrySearchIndex(entry: SearchEntry): EntrySearchIndex {
  const annotation = normalizeSearchValue(entry.annotation || '');
  const tags = normalizeSearchValue(
  ...
  const hexTokens = new Set(
    [hexLower, hexLower.replace(/-/g, ''), ...tokenizeSearch(hexLower, { minLength: 1 })].filter(
      Boolean
    )
  );
...
export function buildQueryContext(rawQuery = ''): QueryContext {
...
export function scoreEntryAgainstQuery(entry: SearchEntry, queryContext: QueryContext): number {
  const index = entry.searchIndex || buildEntrySearchIndex(entry);
  const { query, phrase, primaryTokens, weightedTokens, emojiChars, hexCandidates } = queryContext;
  ...
  for (const [token, weight] of weightedTokens.entries()) {
    ...
    if (index.annotationTokens.has(token)) {
      ...
      findFuzzyToken(token, index.annotationTokens) ||
      findFuzzyToken(token, index.tagTokens)
    ) {
      score += 18 * weight;
      matched = true;
      fuzzyMatches += 1;
...
export function filterAndRankEntries(entries: SearchEntry[], rawQuery: string): SearchEntry[] {
