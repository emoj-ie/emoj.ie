...
export function buildQueryContext(rawQuery = ''): QueryContext {
  ...
  const primaryTokens = queryTokens.filter(
...
export function scoreEntryAgainstQuery(
  entry: SearchEntry,
  queryContext: QueryContext,
): number {
  const index = entry.searchIndex || buildEntrySearchIndex(entry);
  const { query, phrase, primaryTokens, weightedTokens, emojiChars, hexCandidates } =
    queryContext;
...
export function filterAndRankEntries(
  ...
  scored.sort((a, b) => {
    ...
    return String(a.entry.annotation || '').localeCompare(
      String(b.entry.annotation || ''),
      'en',
      { sensitivity: 'base' },
    );
  });
